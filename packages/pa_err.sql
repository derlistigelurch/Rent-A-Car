...
AS
  ...
  /** In: l_n_err_code - Fehlercode
  /** In: l_v_err_msg - Fehlermeldung
  /** Developer: 
  /** Description: Speichert die Fehlermeldung + Fehlercode in eine Tabelle
  /**
  /**********************************************************************/
  PROCEDURE sp_err_handling (l_n_err_code IN NUMBER, l_v_err_msg IN VARCHAR2);
  END pa_err;
/
...
AS
  ...
  AS
    ...
    BEGIN
      ...
      VALUES (err_table_seq.NEXTVAL, l_n_err_code, l_v_err_msg);
      COMMIT;
    END sp_err_handling;
  /*************************************************************************/
END;
